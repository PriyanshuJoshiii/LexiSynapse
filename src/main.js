...
import { buildLexerDFA } from './dfa.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-generate-dfa').addEventListener('click', generateDFA);
  document.getElementById('btn-reset').addEventListener('click', () => {
    ...
    document.getElementById('viz-title').innerText = 'NFA Visualization';
...
function setupCodeEditor() {
  ...
  async function fetchCompletion(codeStr) {
    try {
      // Build active lexical rules context dynamically
      const rulesContext = rules.map(r => `Type: ${r.type}, Pattern: /${r.regex}/`).join('\n');
      ...
      const response = await fetch("http://localhost:11434/api/generate", {
        ...
        body: JSON.stringify({
          ...
          system: `You are a code completion backend for a custom Lexical Analyzer. The user has defined the following token rule syntax:\n${rulesContext}\n\nYour job is to output ONLY the very next characters or words that logically follow the user's input code to complete the line, matching their token rules. Do not repeat the input code. Do not write explanations. Do not use markdown. Output strictly the direct completion characters and nothing else.`,
          prompt: codeStr,
          stream: false,
          options: {
            num_predict: 20,
      ...
      if (!response.ok) {
        console.error("Ollama Local API Error:", response.statusText);
        return "";
      }

      const data = await response.json();
      let suggestion = data.response ? data.response : "";
      
      // If the model repeated the input code prefix, slice it off
      if (suggestion.trim().startsWith(codeStr.trim())) {
        suggestion = suggestion.trim().slice(codeStr.trim().length);
      }
  ...
  function updateSuggestionDisplay(text, suggestion) {
    codeSuggestion.innerHTML = '';
    if (suggestion) {
      // The transparent code string to pad the suggestion exactly to where the cursor is.
      const paddingSpan = document.createElement('span');
      paddingSpan.style.color = 'transparent';
      paddingSpan.textContent = text;

      const suggestionSpan = document.createElement('span');
      suggestionSpan.className = 'text-zinc-600';
      suggestionSpan.textContent = suggestion;

      codeSuggestion.appendChild(paddingSpan);
      codeSuggestion.appendChild(suggestionSpan);
    }
  }
  ...
  input.addEventListener('input', () => {
    ...
    // Debounce LLM API call
    clearTimeout(debounceTimer);
    if (text.trim().length > 0) {
      debounceTimer = setTimeout(async () => {
        const suggestion = await fetchCompletion(text);
        if (suggestion && input.value === text) { // ensure text hasn't changed
          currentSuggestion = suggestion;
          updateSuggestionDisplay(text, suggestion);
        }
      }, 500); // 500ms delay
    }
  });
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Tab' && currentSuggestion) {
      ...
      currentSuggestion = '';
      input.dispatchEvent(new Event('input')); // trigger highlight and clear suggestion
    }
  });

  const syncScroll = function (e) {
    const preHighlight = codeHighlight.parentElement;
    preHighlight.scrollTop = this.scrollTop;
    preHighlight.scrollLeft = this.scrollLeft;

    const preSuggestion = codeSuggestion.parentElement;
    preSuggestion.scrollTop = this.scrollTop;
    preSuggestion.scrollLeft = this.scrollLeft;
  };
...
function generateDFA() {
  const loader = document.getElementById('nfa-loading');
  loader.classList.remove('hidden');
  document.getElementById('viz-title').innerText = 'DFA Visualization';

  setTimeout(() => {
    try {
      const nfaData = buildLexerNFA(rules);
      const dfaData = buildLexerDFA(nfaData, rules);
      visualizer.draw(dfaData);
    } catch (err) {
      alert("Error building DFA! Check regex syntax.");
      console.error(err);
    } finally {
      loader.classList.add('hidden');
    }
...
function setupAIChatbot() {
  ...
  const presetChips = document.querySelectorAll('.chat-chip');

  let isChatOpen = false;
  let isSettingsOpen = false;

  // Load existing Gemini API Key from LocalStorage
  ...
  saveKeyBtn.addEventListener('click', () => {
    ...
    saveKeyBtn.className = 'px-3 py-1.5 bg-emerald-600 text-white font-medium text-xs rounded-lg transition-colors cursor-pointer shrink-0';
    
    setTimeout(() => {
      saveKeyBtn.textContent = originalText;
      saveKeyBtn.className = 'px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs rounded-lg transition-colors cursor-pointer shrink-0';
      isSettingsOpen = false;
      settingsPanel.classList.remove('active');
    }, 1000);
  });
  ...
  async function sendMessage(text) {
    ...
    if (!geminiApiKey) {
      ...
      appendMessage("🤖 <strong>Lexi:</strong> Please click the settings gear (⚙️) in the chat header and paste your <strong>Gemini API Key</strong> to use the assistant.", 'ai');
      messagesContainer.scrollTop = messagesContainer.scrollHeight;
      return;
    }
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${geminiApiKey}`, {
        ...
        body: JSON.stringify({
          systemInstruction: {
            parts: [
              {
                text: "You are Lexi, a helpful AI compiler design and token pattern teaching assistant. Help the user write custom Regular Expressions, NFAs, DFAs, and lexical rule tokens. Be concise, highly encouraging, and explain things clearly. When displaying regex patterns, write the exact regex inside code blocks, e.g. `[a-zA-Z]+`, and explain the components simply."
              }
            ]
          },
          contents: [
      ...
      if (!response.ok) {
        ...
        appendMessage(`Gemini API Error: ${errData.error?.message || response.statusText}`, 'ai');
        return;
      }

      const data = await response.json();
      const aiResponse = data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0]
        ? data.candidates[0].content.parts[0].text.trim()
        : "I couldn't generate an answer from Gemini.";
      appendMessage(aiResponse, 'ai');
    } catch (err) {
      typingIndicator.remove();
      appendMessage("Unable to connect to the Google Gemini API server. Please check your internet connection.", 'ai');
  ...
  // Preset suggestion chips handlers
  presetChips.forEach(chip => {
    chip.addEventListener('click', () => {
      let query = "";
      if (chip.textContent.includes("Float")) {
        query = "Show me the regular expression for float numbers (e.g. 3.14 or .05) and explain it.";
      } else if (chip.textContent.includes("Hexadecimal")) {
        query = "What is the regex to match hexadecimal constants (e.g. 0x1A2F or 0X9B) in programming languages?";
      } else if (chip.textContent.includes("String")) {
        query = "What is a regular expression to match double-quoted string literals with escape support?";
      } else {
  ...
  function appendMessage(text, sender) {
    ...
    bubble.className = sender === 'user'
      ? 'flex items-start gap-2.5 max-w-[85%] ml-auto justify-end animate-fade-in-up'
      : 'flex items-start gap-2.5 max-w-[85%] animate-fade-in-up';

    const avatar = sender === 'user' ? '' : '<div class="w-6 h-6 rounded bg-zinc-800 flex items-center justify-center shrink-0 text-[10px] text-blue-400 font-bold border border-zinc-700">🤖</div>';
    
    // Formatting text (bolding, newlines, basic markdown code blocks)
    let formattedText = escapeHTML(text)
      .replace(/\n/g, '<br>')
      .replace(/```([a-zA-Z]*)([\s\S]*?)```/g, '<pre class="bg-zinc-950 border border-zinc-850 p-2.5 rounded-lg font-mono text-[11px] text-zinc-300 my-2 overflow-x-auto whitespace-pre text-left">$2</pre>')
      .replace(/`([^`]+)`/g, '<code class="bg-zinc-950 px-1 py-0.5 rounded font-mono text-blue-300 text-[11px]">$1</code>');

    const contentClass = sender === 'user'
      ? 'bg-blue-600 border border-blue-500 rounded-2xl rounded-tr-none px-3.5 py-2.5 text-xs text-white leading-relaxed shadow-sm text-left'
      : 'bg-zinc-900 border border-zinc-800/80 rounded-2xl rounded-tl-none px-3.5 py-2.5 text-xs text-zinc-300 leading-relaxed shadow-sm text-left';
  ...
  // Helper to append typing bubble
  function appendTypingIndicator() {
    const bubble = document.createElement('div');
    bubble.className = 'flex items-start gap-2.5 max-w-[85%]';
    bubble.innerHTML = `
      <div class="w-6 h-6 rounded bg-zinc-800 flex items-center justify-center shrink-0 text-[10px] text-blue-400 font-bold border border-zinc-700">🤖</div>
      <div class="bg-zinc-900 border border-zinc-800/80 rounded-2xl rounded-tl-none px-4 py-3 text-xs text-zinc-300 leading-relaxed shadow-sm flex items-center gap-1">
